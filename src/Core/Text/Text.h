...
namespace oakvr::core
{
	...
	inline auto AddFontFace(std::string fileName)  -> void 
	{
		oakvr::core::Text::GetInstance().AddFontFace(oakvr::core::GetResource(fileName));
	}
}

namespace oakvr::render
{
	inline auto DrawText(std::string text, oakvr::math::Vector3 position, oakvr::render::Color color, std::string fontName, float scale = 1.0f, float angle = 0.0f) -> void
	{ 
		oakvr::core::Text::GetInstance().RenderText(text, position, color, fontName, scale, angle);
	}
