...
bool CFrustum::Inside(const CVector3 &point) const
{
...
bool CFrustum::Inside(const CVector3 &point, float fRadius) const
{
	for(int i = 0; i < eFP_NumPlanes; ++i)
...
bool CFrustum::Inside(const CAABB &aabb) const
{
	for(int i = 0; i < eFP_NumPlanes; ++i)
...
bool CFrustum::Inside(const CAABB &aabb, float fRadius) const
{
	for(int i = 0; i < eFP_NumPlanes; ++i)
...
bool CFrustum::CompletlyInside(const CAABB &aabb) const
{
	for(int i = 0; i < eFP_NumPlanes; ++i)
...
bool CFrustum::CompletlyInside(const CAABB &aabb, float fRadius) const
{
	for(int i = 0; i < eFP_NumPlanes; ++i)
...
void CFrustum::Init(float fFOV, float fAspect, float fNear, float fFar)
{
	m_fFOV = fFOV;
	m_fAspect = fAspect;
	m_fNear = fNear;
...
void CFrustum::ComputePlanesFromMatrix(const CMatrix &m)
{
